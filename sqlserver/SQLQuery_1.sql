...
-- description varchar(50),yearsofpayements int,amount float,maturityperiod int,
-- maturityamount float,validity int);
...
-- user_id int references user_details(user_id),policy_no varchar(20) references user_policies(policy_no),dateofpayment date,amount float,fine float);
...
JOIN policy_sub_types pst 
    ON rpt.policy_type_code = pst.policy_type_code
